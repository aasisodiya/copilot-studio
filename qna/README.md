...
1. How to set or define a list variable in Copilot Studio?
   ...
   - Then inside `To value` select `Formula` tab then in `fx` you can define list as follows
...
2. How to save inputs from Adaptive Card into Dataverse Table?
   ...
   - Then in the same block, use `Add an input` button to define all the variables that the agent flow will receive. These variables will hold the data to be saved in the Dataverse table row.
